...
```{r}
#Task 4
#1. What is the square root of 10? I`m use the command for square

sqrt(10)

#2. What is the logarithm of 32 to the base 2?I`m use command log but in added base 2 inside the operation

log(32,base=2)

#3. What is the sum of the numbers from 1 to 1000?Im use command sum but and use : because if im use , only take 1 and 1000

sum(1:1000)

#4. What is the sum of all even numbers from 2 to 1000? Im use 2 more commands seq and by. By because i need only pairs so this sum 2 and seq number generation
...
a <- data %>%
  ...
  geom_histogram(alpha = 1, position = "identity", bins = 15) +
  labs(title = "Length distribution by species", x = "Length", y = "Count") +
  theme_minimal()

#histogram species=jedi
b <- hist(data$length[data$species == "jedi"],
     main = "Length distribution - Jedi",
     xlab = "Length", breaks = 10, col = "green")
#histogram species=sith
c <-hist(data$length[data$species == "sith"],
     main = "Length distribution - Sith",
     xlab = "Length", breaks = 10, col = "purple")
